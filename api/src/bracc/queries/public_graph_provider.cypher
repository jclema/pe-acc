MATCH (center:Provider)
WHERE elementId(center) = $provider_id
   OR center.ruc = $provider_identifier
...
WHERE length(p) <= $depth
  ...
  AND (
    n:Provider OR n:Entity OR n:ProcurementProcess OR n:Award
    OR n:BudgetExecution OR n:Sanction OR n IS NULL
  )
